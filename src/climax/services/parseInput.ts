...
export const parseInput = (config: EditorConfig, input: string, range?: Range): string => {
    const { rules } = config;

    let output = rules.reduce((acc, rule) => parseForRule(rule, acc, range), input);
...
const parseForRule = (rule: ParsingRule, output: string, range?: Range): string => {
